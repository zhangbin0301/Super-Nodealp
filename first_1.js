...
async function runnpm() {
    ...
    try {
        ...
        try {
            if (NEZHA_VERSION === 'V0') {
                await execPromise(`nohup ${FILE_PATH}/${fileMapping['npm']} -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --report-delay=4 --skip-conn --skip-procs --disable-auto-update >/dev/null 2>&1 &`);
            } else if (NEZHA_VERSION === 'V1') {
